...
const addToCart = ( productId, selectedOptionObj, query, mode ) => async (dispatch) => {
  try {
    ...
    const response = await api.post("/cart", {productId, selectedOptionObj, mode});
...
const deleteCartItem = (_id, query, mode) => async (dispatch) => {
  try {
    ...
    if(mode !== "edit") dispatch(commonUiActions.showToastMessage("장바구니 항목 삭제를 완료했습니다.", "success"));
